...
## Preparacion y analisis inicial de datos

#Usaremos los datos de USArrests como conjuntos de datos de demostración, con el ratio de crimenes por estado de USA. Este dataset está incluido en R.
#Usaremos solo un subconjunto de los datos tomando 15 filas aleatorias entre las 50 filas en el conjunto de datos. 
#Esto se hace usando la función **sample()**. A continuacion, estandarizamos los datos utilizando la funcion **scale()**:



# Cargamos librerias
# factoextra: para visualizacion de clusters 
...
#* `dist()` [paquete stats]: Acepta solo datos numericos como entrada.
#* `get_dist()` [paquete factoextra]: Acepta solo datos numéricos como entrada. 
#Admite medidas basadas en el cálculo de correlación (*pearson*, *kendall* y *spearman*).
#* `daisy()` [paquete cluster]: Admite variables nominales u ordinales. En ese caso, se utilizará automáticamente 
#el coeficiente de Gower (? Daisy para ver en que consiste este coeficiente).
...
# Cargamos libreria para utilizar algoritmos de clustering
library(cluster)
...
#La función *k-means* es muy simple de usar, se encuentra en el paquete `stats`. Tiene los siguientes argumentos
...
#  nstart: Número de centroides iniciales generados cuando el parámetro `centers`es un número. Se recomienda  nstart > 1.
...
#Veamos el agrupamiento producido con k=3. Vamos a visualizar un gráfico de dispersión cuyos dos ejes son dos variables del conjunto de datos, 
#y a cada muestra se le dará **un color** *de acuerdo al grupo al que se le ha asignado*, finalmente, mostramos el modelo obtenido.
...
mdl$cluster #muestro clusters
mdl$centers #muestro centroides
mdl$totss #errores totales
mdl$withinss #suma de errores dentro del cluster
mdl$betweenss ##suma de errores entre clusters
mdl$tot.withinss #suma total entre withins y betweens
mdl$size
mdl$iter
#Tambien podemos representar las agrupaciones de forma global utilizando la funcion fviz_cluster()
...
#  geom: Especificacion de la geometria utilizada, "point", "text" o c("point", "text")
...
#Otra funcion que se puede utilizar es la funcion `clusplot()`del paquete *cluster*. 
#Esta funcion, como la anterior, representa el agrupamiento con respecto a sus 2 componente principales.


clusplot(df, mdl$cluster, color=TRUE, shade=TRUE, 
   labels=2, lines=0)

#Tambien podemos representar a que cluster 
#pertenece cada punto.
library(fpc)
plotcluster(df, mdl$cluster)

#Vamos ahora a estudiar el efecto del número de iteraciones. 
#En los siguientes ejemplos vemos tres llamadas diferentes a *k-means* 
#variando los parámetros: solo se realiza una reasignación aleatoria y el número de iteraciones máxima es 1 o 2 o 3.
...
#Veamos el efecto de más repeticiones de asignacion inicial:
mdl = kmeans(df, centers=3, nstart = 10, iter.max = 1) 
fviz_cluster(mdl, data=df)
mdl = kmeans(df, centers=3, nstart = 10, iter.max = 2) 
fviz_cluster(mdl, data=df)
mdl = kmeans(df, centers=3, nstart = 10, iter.max = 3) 
fviz_cluster(mdl, data=df)



#Y ahora proponiendo mas iteraciones...
mdl2 = kmeans(df, centers=3, nstart = 10, iter.max = 100)
fviz_cluster(mdl, data=df)

#Finalmente, escalaremos primero el dataset y 
#luego aplicaremos el agrupamiento.
#ahora, escalemos a [0.0, 1.0] el dataset:
th = apply(df, 2, min) #busca minimo por cada vble por columna (2=columna)
...
## Busqueda del cluster optimo
#El siguiente paso consiste en evaluar los diferentes candidatos de tamaño de cluster. 

#Existen muchos metodos para intentar buscar el numero cluster optimo. Todos ellos se basan en aplicar el algoritmo de clustering con distintos valores de k.
...
#Esto se conoce como regla del codo. Para ello utilizamos la función `fviz_nbclust`.
...
# method: Metodo para determinar el numero de clusters

#Vamos a probarlo con `kmeans` con k entre 1 y 10.

#estos valores de k que van de 1 al 10 son valores por defecto del fviz_nbclust
#para ver mas cosas hago help("fviz_nbclust")
#se definen como min.nc, max.nc por ejemplo: min.nc = 2, max.nc = 10

set.seed(123)
res<- fviz_nbclust(df, kmeans, method = "wss") #wss es el param a optimizar
res

#para elejir un k segun el teorema del codo, es decir que
#cuando se tiene un descenso brusco del error
#y luego empieza a estabilizarse, se elige el que esta en el "codo"
#en este caso es el 4

#Tambien podemos usar la funcion `NbClust`, que computa un metodo de clustering con *k* clusters entre *min.nc* y *max.nc*, utilizando como distancia la especificada en distance. Su sintaxis es la siguiente:
...
# min.nc: Numero minimo de clusters
#\item  max.nc: Numero maximo de clusters
#\item  method: Método de cluster usado: "ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median", "centroid", "kmeans".
#\item  Index: Indice de validacion usado: "kl", "ch", "hartigan", "ccc", "scott", "marriot", "trcovw", "tracew", "friedman", "rubin", "cindex", "db", "silhouette", "duda", "pseudot2", "beale", "ratkowsky", "ball", "ptbiserial", "gap", "frey", "mcclain", "gamma", "gplus", "tau", "dunn", "hubert", "sdindex", "dindex", "sdbw", "all" (todos menos GAP, Gamma, Gplus and Tau), "alllong" (todos).
#todos los indices de validacion tienen encuenta la cohesion
library(NbClust)

#indice de validacion entre 2 y 10

res.nbclust <- NbClust(df, distance = "euclidean", min.nc = 2, max.nc = 10, 
                       method = "complete", index ="all") 


#Vamos a realizar la agrupacion con el numero de clusters optimo

set.seed(123)
km.res <- kmeans(df, 2, nstart = 25)
print(km.res)

km.res$totss #error total
...
#Para ellos usamos la funcion silueta.
#matriz de distancia dist(df)
Sil.km <- silhouette(km.res$cluster, dist(df))

plot(Sil.km,  col = 2:5) #col son los colores
...
si.sum #muestra datos estadisticos b?sicos
...
#Por otro lado, es posible comparar dos agrupaciones distintas. 
#La función `cluster.stats()` del paquete *fpc* proporciona un mecanismo para comparar la similitud de dos soluciones de clúster 
#usando una variedad de criterios de validación (coeficiente gamma de Hubert, el índice de Dunn y el índice de rand corregido)
statistics<-cluster.stats(df, mdl$cluster, mdl2$cluster)
statistics

################################################
## Otros metodos de particionado: PAM y CLARA #
###############################################
#PAM se puede realizar SOLO con datos donde todas las variables sean continuas.
#La funcion standard de R que permite utilizar el método PAM (Particionado alrededor de la madiana) es `pam()` (en paquete cluster). Su sintaxis es:
...
#El metodo `hclust()` es un metodo de **Clustering Aglomerativo**. Su sintaxis es:
...
#A continuación se agruparán las variables usando el metodo average:
...
plot(hc, hang = -1) #hang alinea todo en la misma linea


#Vamos a cortar elárbol en 10 agrupaciones, 
#con la funcion `cutree()`.
#aca se usa el metodo del crentroide
...
#En la variable **memb** se almacena el 
#numero de cluster asignado a cada ejemplo (en este caso un numero entre 1 y 5). Si queremos conocer los elementos de un cluster:

rownames(USArrests)[memb == 2] #pregunto q ciudades estan en el cluster 2
rownames(USArrests)[memb == 1] #pregunto q ciudades estan en el cluster 1
rownames(USArrests)[memb == 5] #pregunto q ciudades estan en el cluster 5
rownames(USArrests)[memb == 3] #pregunto q ciudades estan en el cluster 3

#A continuacion se realizara usando el cuadrado de la distancia Euclidea, cortando los árboles en 10 agrupaciones y reconstruyendo la parte superior del árbol a partir de los centros de los clusters:
...
rownames(USArrests)[memb == 2] #cuando tengo clusteres con 1 o dos elementos no es bueno
...
#Ahora vamos a comparar diferentes metodos para realizar el clusterizado jerárquico:
#metodos aglomerativos con distintos metodos de linkado
...
#En general el indice de ward es el que mejor funciona.


#Comparemos ahora los resultados al usar diferentes medidas de distancia, 
#todos para el metodo single:
single.euclidean <- hclust(dist(attitude), method = 'single') #sin nada es euclidiana
...
# Uso de la funcion entanglement() para comparar dos dendogramas. 
#Cuanto mas proimo a 0 mas se parecen.
...
#Vamos a utilizar la funcion dbscan de los paquetes fpc y dbscan para realizar clustering. Observa el objeto que retornan. ¿Producen el mismo agrupamiento?. A continuación dibujamos  los resultados utilizando la función fviz_cluster (En el siguiente ejemplo db es el objeto que retorna dbscan y df un data.frame con los datos)


#La funcion `dbscan()` del paquete **fpc** es una implementacion de DBSCAN (Density-based spatial clustering of applications with noise) que usa un tipo especial de árbol llamado *kd-tree*. Esta implementación es significativamente más rápido y puede trabajar con conjuntos de datos mucho mayores que  **dbscan** del paquete **dbscan**.

#La sintaxis de esta funcion (en ambos paquetes) es 
...
#para ver dentro del cluster se hace
db2$cluster
#cluster 0 es el ruido
...
#aca se hace lo mismo que arriba pero escalando los datos con scale(df)
#es decir normalizando datos
db3 <- fpc::dbscan(scale(df), eps = 0.15, MinPts = 5)
fviz_cluster(db3, data = df, stand = FALSE,
             ellipse = FALSE, show.clust.cent = FALSE,
             geom = "point",palette = "jco", ggtheme = theme_classic())

#cuando se utiliza un metodo que calcula distancia es aconsejable normalizar
#pero si se tiene otro paradigma quizas no sea la mejor opcion
...
d5<-dbscan::kNN(df,5) #los 5 mas cercanos
d5d<-dbscan::kNNdist(df,5) #las 5 distancias mas cercanas

#head(d5$id)

par(mfrow=c(1,1))
dbscan::kNNdistplot(df,5)
abline(h=0.5, lty=2) #marco lineas rectas
abline(h=0.4, lty=3)
...
db0.1 <- fpc::dbscan(df, eps = 0.15, MinPts = 5)
...
#no olvidar que el epsilon (eps) es el diametro
#serializacion de lo de arriba:
